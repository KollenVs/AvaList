...
class UserRating(Module):
    ...
    async def update_act(self):
        users = {}
        max_uid = await self.server.redis.get("ids")
        if not max_uid:
            return
        max_uid = int(max_uid)
        for i in range(1, max_uid+1):
            act = await self.server.redis.get(f"id:{i}:act")
    ...
    async def update_hrt(self):
        ...
        max_uid = int(max_uid)
        for i in range(1, max_uid+1):
            hrt = await self.server.redis.get(f"id:{i}:hrt")
        ...
        i = 1
        for user in sorted_users:
            cr = int(await self.server.redis.get(f"id:{user[0]}:crt"))
