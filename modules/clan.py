...
class Clan(Module):
    ...
    async def create(self, msg, client):
        r = self.server.redis
        has_clan = await r.get(f"id:{client.uid}:clan")
        if has_clan:
            return
        ...
        pipe.set(f"id:{client.uid}:clan", cid)
        await pipe.execute()
        cinv = {"c": {"cfrn": {"id": "cfrn", "it": []},
                      "crc": {"id": "crc", "it": []}}}
        act = await r.get(f"id:{client.uid}:act")
        if act:
            act = int(act)
        else:
            act = 0
        members = {client.id: {"uid": client.uid, "jd": create_date, "rl": 3,
                                "cam": {"ah": [], "cid": 1, "ap": act},
                                "cid": cid, "ccn": 0}}
    ...
    async def load_clans(self, msg, client):
        ...
        for cid in msg[2]["cids"]:
            ...
            for member in clan["members"]:
                act = await self.server.redis.get(f"id:{member}:act")
                if act:
                    act = int(act)
                else:
                    act = 0
    ...
    async def join_clan(self, cid, uid):
        ...
        if uid in self.server.online:
            ...
            loop = asyncio.get_event_loop()
            loop.create_task(refresh_avatar(tmp, self.server))
        act = await self.server.redis.get(f"id:{uid}:act")
        if act:
            act = int(act)
        else:
            act = 0
    ...
    async def donateClan(self, msg, client):
        ...
        await self.server.redis.decrby(f"id:{client.uid}:gld", outcomeGold)
