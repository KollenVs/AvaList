...
class ClanRequest(Module):
    ...
    async def load_requests(self, msg, client):
        cid = await self.server.redis.get(f"id:{client.uid}:req")
        if not cid:
    ...
    async def create_request(self, msg, client):
        r = self.server.redis
        if await r.get(f"id:{client.uid}:clan"):
            return
        if await r.get(f"id:{client.uid}:req"):
            return
        cid = msg[2]["cid"]
        if str(cid) not in await r.smembers("clans"):
            return
        await r.sadd(f"clans:{cid}:req", client.uid)
        await r.set(f"id:{client.uid}:req", cid)
    ...
    async def delete_request(self, msg, client):
        r = self.server.redis
        rid = msg[2]["rid"]
        if str(rid) == client.id:
            cid = await r.get(f"id:{client.uid}:req")
        else:
            cid = await r.get(f"id:{client.uid}:clan")
        if not cid:
            return
