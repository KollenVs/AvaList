...
class ChatDecor(Module):
    ...
    async def save_chat_decor_model(self, msg, client):
        ...
        if not bubble:
            await r.delete(f"id:{client.uid}:bubble")
        else:
            await r.set(f"id:{client.uid}:bubble", bubble)
        text_color = msg[2]["chtdc"]["tcl"]
        if not text_color:
            await r.delete(f"id:{client.uid}:tcl")
        else:
            await r.set(f"id:{client.uid}:tcl", text_color)
        chat_bubble = msg[2]["chtdc"]["bt"]
        if chat_bubble in const.PREMIUM_BUBBLES and not user_data["premium"]:
            return await client.send(["cp.ms.rsm", {"txt": "Приобретите премиум-статус либо Элиту"}])
        if not chat_bubble:
            await r.delete(f"id:{client.uid}:chat_bubble")
        else:
            await r.set(f"id:{client.uid}:chat_bubble", chat_bubble)
        bubble = await r.get(f"id:{client.uid}:bubble")
        text_color = await r.get(f"id:{client.uid}:tcl")
        chat_bubble = await r.get(f"id:{client.uid}:chat_bubble")
