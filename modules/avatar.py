...
class Avatar(Module):
    ...
    async def appearance(self, msg, client):
        ...
        if subcommand == "rnn":
            ...
            if len(name) > const.MAX_NAME_LEN:
                return
            await self.server.redis.lset(f"id:{client.uid}:appearance",
                                         0, name)
            ...
            if not current_apprnc:
                ...
                await self.server.redis.set(f"id:{client.uid}:wearing",
                                            "casual")
                if apprnc["g"] == 1:
                ...
                if user_data["role"] >= 4:
                    await self.server.inv[client.uid].add_item("survivedChatBubble", "gm")
                if user_data["premium"]:
                    await self.server.inv[client.uid].add_item("tgcChatBubble", "gm")
                for item in weared:
    ...
    async def change_ctp(self, uid, new_ctp):
        ctp = await self.server.redis.get(f"id:{uid}:wearing")
        if ctp == new_ctp:
            return
        await self.server.redis.set(f"id:{uid}:wearing", new_ctp)
    ...
    async def wear_cloth(self, msg, client):
        ...
        wearing = await self.server.redis.smembers(f"id:{client.uid}:{ctp}")
    ...
    async def buy_clothes(self, command, clothes, ctp, client):
        items = await self.server.redis.smembers(f"id:{client.uid}:items")
        ...
        pipe.set(f"id:{client.uid}:gld", user_data["gld"] - gold)
        pipe.set(f"id:{client.uid}:slvr", user_data["slvr"] - silver)
        pipe.set(f"id:{client.uid}:crt", user_data["crt"] + rating)
        await pipe.execute()
        await self.change_ctp(client.uid, ctp)
        for cloth in to_buy:
    ...
    async def buy_clothes_suit(self, tpid, ctp, client):
        ...
        if user_data["gld"] < gold or user_data["slvr"] < silver:
            return
        await self.server.redis.set(f"id:{client.uid}:gld",
                                    user_data["gld"] - gold)
        await self.server.redis.set(f"id:{client.uid}:slvr",
                                    user_data["slvr"] - silver)
        await self.server.redis.set(f"id:{client.uid}:crt",
    ...
    async def update_appearance(self, apprnc, client):
        ...
        await redis.delete(f"id:{client.uid}:appearance")
        await redis.rpush(f"id:{client.uid}:appearance", nick,
    ...
    async def update_crt(self, uid):
        redis = self.server.redis
        clothes = []
        for tmp in await redis.smembers(f"id:{uid}:items"):
            if await redis.lindex(f"id:{uid}:items:{tmp}", 0) == "cls":
                if "_" in clothes:
            ...
            for _category in self.clothes_list[gender]:
                for item in self.clothes_list[gender][_category]:
                    if item == cloth:
                        ...
                        crt += item["rating"]
                        break
        await self.server.redis.set(f"id:{uid}:crt", crt)
        return crt
