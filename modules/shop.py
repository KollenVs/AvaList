...
class Shop(Module):
    ...
    async def buy(self, client, item, count=1, add=True):
        ...
        await redis.set(f"id:{client.uid}:gld", user_data["gld"]-gold)
        await redis.set(f"id:{client.uid}:slvr", user_data["slvr"]-silver)
        if add:
            await self.server.inv[client.uid].add_item(item, "gm", count)
            cnt = int(await redis.lindex(f"id:{client.uid}:items:{item}", 1))
