...
class House(Location):
    ...
    def __init__(self, server):
        ...
        self.commands.update({"minfo": self.get_my_info, "gr": self.get_room,
                              ...
                              "ioinfo": self.init_owner_info, "lkhs": self.close_house, "ulkhs": self.open_house})

    async def close_house(self, msg, client):
        lt = msg[2]["lt"]
        return await client.send(["h.lkhs", {}])

    async def open_house(self, msg, client):
        return await client.send(["h.ulkhs"])
    ...
    async def get_my_info(self, msg, client):
        ...
        survive = await self.server.redis.get(f"id:{client.uid}:suchle")
        tcgchat = await self.server.redis.get(f"id:{client.uid}:tgcchle")
        ...
        for item in self.server.achievements:
            ac[item] = {"p": 0, "nWct": 0, "l": 1, "aId": item}
        tr = {}
    ...
    async def _perform_login(self, client):
        ...
        black_list = await self.server.redis.lrange(f"id:{client.uid}:ban_chat", 0, -1)
        await client.send(["cp.cht.gbl", {"blcklst": {"uids": [black_list]}}])
        await client.send(["nws.hasnews", {"gnexst": False, "gnunr": False}])
        await self.server.modules["fa20"].get_info(client.uid, client)
