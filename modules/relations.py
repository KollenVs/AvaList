...
class Relations(Module):
    ...
    def __init__(self, server):
        ...
        self.commands = {"get": self.get_relations,
                         ...
                         "rings": self.rings,
                         "ednt": self.edit_note, "urp": self.add_progress}
    ...
    async def _remove_relation(self, link):
        for uid in self.server.online:
            await self.server.online[uid].send(["rl.rmv", {"uid": uid}])
    ...
    async def _get_relation(self, uid, link):
        ...
        pipe.get(f"id:{uid}:note:{second_uid}")
    ...
    async def buy_ring(self, ring, uid):
        ...
        if price["slvr"] > user_data["slvr"] or \
           price["gld"] > user_data["gld"]:
            return False
        r = self.server.redis
        await r.set(f"id:{uid}:slvr", user_data["slvr"] - price["slvr"])
        await r.set(f"id:{uid}:gld", user_data["gld"] - price["gld"])
        return True
