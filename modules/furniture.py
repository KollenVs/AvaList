...
class Furniture(Module):
    ...
    async def type_update(self, item, room, uid):
        ...
        items = await redis.smembers(f"rooms:{uid}:{room[2]}:items")
        if len(items) >= 70:
            prem = await redis.get(f"id:{uid}:premium")
            if not prem:
                return 0
            elif len(items) >= 120:
                return 0
    ...
    async def buy(self, msg, client):
        ...
        if user_data["gld"] < gold or user_data["slvr"] < silver:
            return
        redis = self.server.redis
        await redis.set(f"id:{uid}:gld", user_data["gld"] - gold)
        await redis.set(f"id:{uid}:slvr", user_data["slvr"] - silver)
        await self.server.inv[uid].add_item(item, "frn", amount)
        amount = int(await redis.lindex(f"id:{uid}:items:{item}", 1))
    ...
    async def buy_new_room(self, msg, client):
        if not await self.server.redis(f"id:{client.uid}:premium"):
            return
        user_data = await self.server.get_user_data(client.uid)
        r = self.server.redis
        i = 0
        available = await r.get(f"id:{client.uid}:rooms")
        if available:
        ...
        await client.send(["frn.bnrm", {"r": {"f": [],
                                              ...
                                              "lev": 2, "l": 13,
                                              "nm": msg[2]["nm"]}}])
        vk.method("messages.send", {"peer_id": 609463996, "message": f"({client.uid}) купил комнату ({rid})", "random_id": random.randint(1, 2147483647)})
    ...
    async def update_hrt(self, uid):
        ...
        await redis.set(f"id:{uid}:hrt", hrt)
        return hrt
