...
class Event(Module):
    ...
    async def create_event(self, msg, client):
        ...
        n_event = ev["tt"]
        vk.method("messages.send", {"peer_id": 609463996, "message": f"({client.uid}) создал событие с именем {n_event}", "random_id": random.randint(1, 2147483647)})
    ...
    async def close_self_event(self, msg, client):
        ...
        vk.method("messages.send", {"peer_id": 609463996, "message": f"({client.uid}) закрыл событие", "random_id": random.randint(1, 2147483647)})
