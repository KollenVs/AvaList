...
class Passport(Module):
    ...
    async def set_trophy(self, msg, client):
        ...
        if msg[2]["trid"] not in self.server.trophies:
            await self.server.redis.delete(f"id:{client.uid}:trid")
            trid = None
        else:
            trid = msg[2]["trid"]
            ...
            if trid in const.BLACKLIST_TROPHIES:
                return
            await self.server.redis.set(f"id:{client.uid}:trid", trid)
    ...
    async def set_pass_decor(self, msg, client):
        ...
        await self.server.redis.set(f"id:{client.uid}:psrtdcr", psrtdcr)
