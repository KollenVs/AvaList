...
class Inventory():
    ...
    async def add_item(self, name, type_, amount=1):
        ...
        item = await redis.lrange(f"id:{self.uid}:items:{name}", 0, -1)
        if item:
            if type_ == "cls":
                return
            await redis.lset(f"id:{self.uid}:items:{name}", 1,
                             int(item[1])+amount)
            ...
            await redis.sadd(f"id:{self.uid}:items", name)
            await redis.rpush(f"id:{self.uid}:items:{name}", type_, amount)
            type_items = self.inv["c"][type_]["it"]
            type_items.append({"c": amount, "tid": tid, "iid": iid})

    async def take_item(self, item, amount=1):
        redis = self.server.redis
        items = await redis.smembers(f"id:{self.uid}:items")
        if item not in items:
            return False
        tmp = await redis.lrange(f"id:{self.uid}:items:{item}", 0, -1)
        if not tmp:
            await redis.srem(f"id:{self.uid}:items", item)
            return False
        type_ = tmp[0]
        have = int(tmp[1])
        del tmp
        ...
        type_items = self.inv["c"][type_]["it"]
        if have > amount:
            await redis.lset(f"id:{self.uid}:items:{item}", 1, have - amount)
            for tmp in type_items:
                if tmp["tid"] == item:
                    tmp["c"] = have - amount
                    break
        else:
            await redis.delete(f"id:{self.uid}:items:{item}")
            await redis.srem(f"id:{self.uid}:items", item)
            for tmp in type_items:
                if tmp["tid"] == item:
    ...
    async def get_item(self, item):
        ...
        if item not in items:
            return 0
        have = int(await redis.lindex(f"id:{self.uid}:items:{item}", 1))
        return have

    async def change_wearing(self, cloth, wearing):
        redis = self.server.redis
        if not await redis.lindex(f"id:{self.uid}:items:{cloth}", 0):
            not_found = True
        else:
            not_found = False
        if "_" in cloth:
        ...
        ctp = await redis.get(f"id:{self.uid}:wearing")
        if wearing:
            if not_found:
            ...
            for item in type_items:
                if item["tid"] == tid and item["iid"] == iid:
                    ...
                    break
            await redis.sadd(f"id:{self.uid}:{ctp}", cloth)
        else:
            weared = await redis.smembers(f"id:{self.uid}:{ctp}")
            if cloth not in weared:
                logging.info(f"Cloth {cloth} not weared for {self.uid}")
                return
            if not not_found:
                type_items.append({"c": 1, "iid": iid, "tid": tid})
            await redis.srem(f"id:{self.uid}:{ctp}", cloth)
    ...
    async def _get_inventory(self):
        self.inv = {"c": {"frn": {"id": "frn", "it": []},
                          ...
                          "lt": {"id": "lt", "it": []},
                          "cls": {"id": "cls", "it": []}}}
        ctp = await self.server.redis.get(f"id:{self.uid}:wearing")
        wearing = await self.server.redis.smembers(f"id:{self.uid}:{ctp}")
        keys = []
        pipe = self.server.redis.pipeline()
        for item in await self.server.redis.smembers(f"id:{self.uid}:items"):
            if item in wearing:
                continue
            pipe.lrange(f"id:{self.uid}:items:{item}", 0, -1)
            keys.append(item)
        ...
        for i in range(len(keys)):
            ...
            if "_" in name:
                ...
                try:
                    ...
                    r = self.server.redis
                    await r.srem(f"id:{self.uid}:items", name)
                    await r.delete(f"id:{self.uid}:items:{name}")
    ...
    async def _check_conflicts(self, cloth):
        ...
        if not category:
            ...
            return
        ctp = await self.server.redis.get(f"id:{self.uid}:wearing")
        weared = await self.server.redis.smembers(f"id:{self.uid}:{ctp}")
