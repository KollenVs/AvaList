...
class Parser():
	...
	def parse_fa20(self):
		info = {"packs": {}, "items": {}, "hats": [], "balloons": []}
		root = etree.parse("config_all_ru/modules/events/firstApril2020.xml",parser=self.parser).getroot()
		for balloon in root.findall(".//balloon"):
			for item in balloon.findall(".//item"):
				info["balloons"].append(item.attrib["type"])
		for hat in root.findall(".//hat"):
			for item in hat.findall(".//item"):
				info["hats"].append(item.attrib["type"])
		for item in root.findall(".//pack[@id]"):
			pack_id = item.attrib["id"]
			price = int(item.attrib["gold"])
			smiles = int(item.attrib["smilesCount"])
			if "bonus" in item.attrib:
				smiles += int(item.attrib["bonus"])
			info["packs"][pack_id] = {"price": price, "smiles": smiles}
		for category in root.findall(".//category[@type]"):
			for item in category.findall(".//item[@id]"):
				info["items"][item.attrib["id"]] = {"price": int(item.attrib["smiles"]), "type": category.attrib["type"]}
		return info
